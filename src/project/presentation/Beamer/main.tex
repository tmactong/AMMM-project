...
\begin{alertblock}
    {Decision Variables}
\centering
\scalebox{0.9}{$Priority_{ij}: \quad Binary \quad \forall i,j \in Members$}\\
\scalebox{0.9}{\hspace{0.9cm} $O_{n}: \quad Integer \quad \forall n \in Members$}\\
\scalebox{0.9}{\hspace{0.25cm} $Income = \sum_{i,j \epsilon Members} Priority_{ij} \cdot m_{ij}$}
\end{alertblock}
...
\scalebox{1}{$\textrm{maximize} \quad Income$}
\end{alertblock}

\begin{alertblock}
    {Constraints}
\scalebox{0.9}{$Priority_{ij} + Priority_{ji} = 1 $}\\
\scalebox{0.9}{$\qquad\qquad\qquad\qquad\forall \quad i,j \in Members \quad \textrm{and} \quad m_{ij} + m_{ji} \neq 0$} \qquad(1)\\
\scalebox{}{}\\
\scalebox{0.9}{$O_{i} + 1 \leq O_{j} + (1 - Priority_{ij}) \cdot N $}\\
\scalebox{0.9}{$\qquad\qquad\qquad\qquad\forall \quad i,j \in Members \quad \textrm{and} \quad m_{ij} + m_{ji} \neq 0$}  \qquad(2)
...
q(<i,j>, S) = \begin{cases}
    ...
    max(Bids_{ij}, Bids_{ji}) & if \; G(S, <i,j>) \textrm{ is a directed acyclic graph}
    \end{cases}
\end{equation*}
}
\end{alertblock}
...
\begin{frame}{Greedy Constructive Phase (data=project.4.dat)}
...
\begin{frame}{Knockon Pairs Selection Process}
...
\While{$improved = $ True}
    ...
    \State $edges^{residual} \gets doTopologicalSort()$
...
\begin{frame}{Local Search Phase (data=project.4.dat)}

\centering
%\animategraphics[controls,width=1\linewidth]{2}{images/local_search_resolution/plot-}{001}{002}
\animategraphics[controls,width=1\linewidth]{2}{images/local_search_resolution/plot-}{001}{036}
...
%\animategraphics[loop,controls,width=1\linewidth]{2}{images/grasp_resolution/plot-}{001}{034}
