...
def get_permutations(digits: typing.Iterable[int]) -> typing.Iterable[str]:
...
def generate_tree(branch: typing.List[int], pairs: typing.List[typing.Tuple[int, int]]) -> typing.Iterable[typing.List[int]]:
    continue_search = False
    for pair in pairs:
        if pair[0] == branch[-1]:
            continue_search = True
            branch.append(pair[1])
            yield from(generate_tree(branch, [x for x in pairs if x != pair and x != pair[::-1]]))
...
def generate_cycles_using_pattern(digits: typing.List[int]) -> typing.Iterable[str]:
    repetitions = len(digits) - 2
    for i in range(repetitions):
        prefix = '1%s' % digits[i + 1]
        moving_digits = (digits[1:i + 1] + digits[i + 2:-1])[:i]
        residual_digits = digits[1:i + 1] + digits[i + 2:]
        permutations = get_permutations_without_using_itertools(residual_digits)
        for permutation in permutations:
            ends_with_moving_digit = False
            for moving_digit in list(map(str, moving_digits)):
                if permutation.endswith(moving_digit):
...
if __name__ == '__main__':
    ...
    # print(20 * "-")
    # generated_sequences = list(generate_cycles_using_pattern(list(range(1, 1 + n))))
